...
def parse_text(article):
    ...
    return [x.lower() for x in tokens if x not in stops]
...
def normalize_word_vecs(tokens, w2v, dim, pos_words=None, neg_words=None, tf_idf=None):
    if pos_words is None or neg_words is None:
        ...
        coeff_pos_words = np.array([10 if x in pos_words else 0 for x in tokens])
        coeff_neg_words = np.array([-10 if x in neg_words else 0 for x in tokens])
...
# function that returns vectorized data of given train/test file
